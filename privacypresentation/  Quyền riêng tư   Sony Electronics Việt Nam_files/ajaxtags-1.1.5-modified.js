...
function arrayToParameterString(array) {
  ...
  for (var i=0; i<array.length; i++) {
    ...
    if (pair.length == 2) {
      str += (i >= 0 ? '&' : '') + pair[0] + '=' + encodeURIComponent(pair[1]);
    } else {
      str += (i >= 0 ? '&' : '') + array[i];
    }
  }
...
function replaceWithValue(sourceString, regExp, element) {
  ...
  switch (element.type) {
    ...
    case 'select-multiple':
      ...
      break;
    default:
      retString = sourceString.replace(regExp, element.innerHTML.replace(/[,]+/g,';comma;'));
      break;
  }
  return retString;
}
...
/*Function used for hiding Autocomplete popup callable from outside*/
function hideAjaxAutocompletePopup(){
	Element.hide("ajaxAutocompletePopup");
...
AjaxJspTag.Autocomplete.prototype = Object.extend (new AjaxJspTag.Base(),{
  ...
  initialize: function(url, options) {
    ...
    this.createPopup();
    this.createIframe();
    if(this.options.sourceElem!=null){
 ...
 setOptions: function(options) {
  ...
  sourceElemChanged: function(e) {
    ...
    //escape
      ...
      if (fieldLength < this.options.minimumCharacters) {
        ...
        //this.options.sourceElem.value=encodeURIComponent(this.options.sourceElem.value);
  ...
  setPopupStyles: function() {
    ...
    if (isIE) {
      maxHeight = 200;
      //because offsetheight never gets a value smaller than maxHeinght on IE we checked scrollHeight 
      //for IE.
      if ($(this.popupElem).scrollHeight < maxHeight) {
      	$(this.popupElem).style.overflow = 'hidden';
	$(this.popupElem).style.height =$(this.popupElem).scrollHeight+ 'px';
    	}else {
    ...
    $(this.popupElem).style.top = (getElementY(this.options.sourceElem)+this.options.sourceElem.offsetHeight) + "px";
    $(this.popupElem).style.left = getElementX(this.options.sourceElem)-1 + "px";
    if (isIE) {
    ...
    //Because of the bug on Mozilla. The original script could not show suggest list
    //in a window with scroll bar.
    ...
    $('layerCover').style.left = getElementX(this.options.sourceElem)-1 + "px";
    $('layerCover').style.width = $(this.popupElem).offsetWidth + "px";
    $('layerCover').style.height = $(this.popupElem).offsetHeight + "px";
...
AjaxJspTag.Callout.prototype = Object.extend (new AjaxJspTag.Base(),{
  ...
  callout: function(xml) {
    ...
    if (items.length > 0) {
      ...
      // hook events
       //	this.targetElem.onmouseleave= this.handleCloseClick.bindAsEventListener(this);
       this.targetElem.onmouseout= this.checkBoxPosition.bindAsEventListener(this);
	  //close callout box of that source element on mouseleave event of each source element
       	for (var i=0; i<this.options.sourceElementList.length; i++) {
	    	//this.options.sourceElementList[i].onmouseleave= this.handleCloseClick.bindAsEventListener(this);
	    	this.options.sourceElementList[i].onmouseout= this.checkBoxPosition.bindAsEventListener(this);
	    }
  ...
  createContainer: function() {
    new Insertion.Top(
      document.getElementsByTagName("body")[0],
        "<div id=\""+this.calloutContainer+"\" style=\"position: absolute; top: 0px; left: 0px\"></div>");
  },
  ...
  constructBox: function() {
    ...
    eBox.setAttribute("style", "position: absolute; top: 0px; left: 0px");
  ...
  moveBox: function(anchor, box) {
    ...
    if (this.options.boxPosition.indexOf("top") >= 0) {
      box.style.top = (posXY[1] - (box.offsetHeight) - 25) + "px";
    } else {
      box.style.top = (posXY[1] + (anchor.offsetHeight) - 2) + "px";
    }
    if (this.options.boxPosition.indexOf("right") >= 0) {
      box.style.left = (posXY[0]/* + 10*/) + "px";
    } else {
      box.style.left = (posXY[0] - (box.offsetWidth) - 23) + "px";
    }
  ...
  checkBoxPosition: function(e) {
   // var outsideContainer = false;
    ...
    // evaluate if cursor is over box and link
    var clickX = e.clientX; // cursor X position
    var clickY = e.clientY + getScrollHeight(); // cursor Y position
    var elemWidth = 0;
   
    if(this.activeElem.offsetWidth >= this.targetElem.offsetWidth){
    	elemWidth = this.activeElem.offsetWidth;
    } else {
    	elemWidth = this.targetElem.offsetWidth;
    }
    
    var boundX1 = parseFloat(this.targetElem.style.left)+5;
    var boundX2 = boundX1 + elemWidth;
    var boundY1 = parseFloat(this.targetElem.style.top)-10;
    var boundY2 = boundY1 + this.activeElem.offsetHeight + this.targetElem.offsetHeight-10;
	//console.log('posX: '+clickX+'\nPosY: '+clickY+'\nboundX1: '+boundX1+'\nboundX2: '+boundX2+'\nboundY1: '+boundY1+'\nboundY2: '+boundY2+'\nscrollHeight: '+getScrollHeight()+'\ncontainerHeight: '+this.targetElem.offsetHeight);
	//console.log(clickX+'<'+ boundX1+'||'+ clickX+'>'+boundX2+'\n'+clickY+'<'+boundY1+'||'+clickY+'>'+boundY2);
		...
		jQuery(window).bind('resize', function() {
		    ...
		    resizeTimer = setTimeout(function(){
		    	if(jQuery.browser.msie){
		    		jQuery('#ajaxAutocompletePopup').css({'left':(inputFocus.offset().left)-2});
		    		jQuery('#layerCover').css({'left':(inputFocus.offset().left)-2});
		    	} else {
		    		jQuery('#ajaxAutocompletePopup').css({'left':inputFocus.offset().left});
		    		jQuery('#layerCover').css({'left':inputFocus.offset().left});
		    	}
		    }, 10);
		}); 
	});	
	
	jQuery('.GlossaryLink, .calloutBox').mouseout(function(){
	//	jQuery('#calloutContainer, .calloutBox').hide();
	});
	
	jQuery('.GlossaryLink, .calloutBox').mouseover(function(){
	//	jQuery('#calloutContainer, .calloutBox').show();
	});
